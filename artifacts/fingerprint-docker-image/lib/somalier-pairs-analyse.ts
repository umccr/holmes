 ...
 *
 * @param pairsResultStreamFactory a factory method that creates a fresh stream reading the pairs result file of somalier
...
export async function pairsAnalyse(
  pairsResultStreamFactory: () => ReadStream,
  ...
  for (const indexSampleId of Object.keys(indexSampleIdToKeyMap)) {
    ...
    const parser = pairsResultStreamFactory().pipe(
      parse({
        delimiter: "\t",
      })
    );
