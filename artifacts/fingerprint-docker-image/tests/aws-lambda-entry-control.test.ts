  ...
  test("basic check of a sample against controls", async () => {
    ...
    // fingerprint files in different orders so we have to be able to handle that
    {
      ...
      for (const l of lines.slice(1)) {
        ...
        if (l.startsWith("00001\ts3://a-bucket/HG002-ready.bam")) {
          expect(l).toBe(
            "00001\ts3://a-bucket/HG002-ready.bam\t-9\t-9\t-9\t-9\t-9\t131.4\t100.5\t5.1\t32.1\t0.43\t0.53\t188\t217\t214\t15850\t0.000\t0.00\t0\t0\t0\t0\t0.00\t0"
          );
          matchCount++;
        }
    ...
    // fingerprint files in different orders so we have to be able to handle that
    {
      ...
      let matchCount = 0;
      for (const l of lines.slice(1)) {
        if (l.startsWith("NA24385\ts3://a-bucket/HG002-ready.bam")) {
          expect(l).toBe(
        ...
        if (l.startsWith("s3://a-bucket/HG002-ready.bam\tNA12878")) {
          expect(l).toBe(
